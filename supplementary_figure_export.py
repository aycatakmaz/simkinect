...
def add_kinect_noise_to_depth(depth_orig, 
        ...
        cv2.imwrite(name_prefix+'figure_depth.png', depth_orig*depth_factor)
        cv2.imwrite(name_prefix+'figure_depth_w_kinect_noise.png', noisy_depth*depth_factor)
        cv2.imwrite('_'.join([name_prefix+'figure_high_contrast',str(scale_factor), str(baseline_m), str(std), str(size_filt)])+'.png', np.hstack((depth_orig*depth_factor, noisy_depth*depth_factor)))

        im_gray = cv2.imread(name_prefix+'figure_depth_w_kinect_noise.png', cv2.IMREAD_GRAYSCALE)
        im_c = cv2.applyColorMap(im_gray, cv2.COLORMAP_MAGMA)
        cv2.imwrite(name_prefix+'figure_depth_w_kinect_noise_im_c.png', im_c)

        im_gray = cv2.imread(name_prefix+'figure_depth.png', cv2.IMREAD_GRAYSCALE)
        im_c = cv2.applyColorMap(im_gray, cv2.COLORMAP_MAGMA)
        cv2.imwrite(name_prefix+'figure_depth_im_c.png', im_c)
...
if __name__ == "__main__":
    ...
    #depth_np_path = 'supp_kinect_w_wo_comparison/scene0000_02_frame_35.npy'
    #depth_np_path = 'supp_kinect_w_wo_comparison/scene0003_02_frame_29.npy'
    #depth_np_path = 'supp_kinect_w_wo_comparison/scene0218_01_frame_21.npy'
    #depth_np_path = 'supp_kinect_w_wo_comparison/scene0218_01_frame_22.npy'
    depth_np_path = 'supp_kinect_w_wo_comparison/scene0218_01_frame_38.npy'
    
    name_prefix = 'supp_kinect_w_wo_comparison/results/'+depth_np_path.split('/')[-1][:-4]+'_'

    img_depth_np = np.load(depth_np_path) #(480, 640), min: 1.2863141, max:inf

    img_depth_np_w_kinect_noise = add_kinect_noise_to_depth(img_depth_np, dot_pattern, name_prefix=name_prefix)
    #pdb.set_trace()
